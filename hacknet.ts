...
type Property = {
    ...
    value: number;
    maxValue: number;
    buyPropertyFunc: BuyPropertyFunc;
    getUpgradeCostFunc: GetUpgradeCostFunc;
};
...
export async function main(ns: NS) {
    ...
    while (counter < threshold) {
        ...
        for (let i = 0; i < ns.hacknet.numNodes(); i++) {
            ...
            const properties: Property[] = [
                {
                    ...
                    value: node.level,
                    maxValue: 200,
                    buyPropertyFunc: ns.hacknet.upgradeLevel,
                    getUpgradeCostFunc: ns.hacknet.getLevelUpgradeCost,
                },
                {
                    property: "Core",
                    value: node.cores,
                    maxValue: 16,
                    buyPropertyFunc: ns.hacknet.upgradeCore,
                    getUpgradeCostFunc: ns.hacknet.getCoreUpgradeCost,
                },
                {
                    property: "Ram",
                    value: node.ram,
                    maxValue: 64,
                    buyPropertyFunc: ns.hacknet.upgradeRam,
                    getUpgradeCostFunc: ns.hacknet.getRamUpgradeCost,
                },
            ];
            for (const property of properties) {
                ...
                upgrades.push({
                    nodeNum: i,
                    property,
                    ...calculateProperty(property, playerMoney, i),
                });
            }
        }
        ...
        if (res) {
            ns.tprint(
                `Upgraded ${chosenUpgrade.property.property} in node #${chosenUpgrade.nodeNum} #${chosenUpgrade.levels} levels to ${chosenUpgrade.property.value + chosenUpgrade.levels}`,
            );
        }
