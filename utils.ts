...
function getAccessToServer(ns: NS, target: string): boolean {
  const portScripts: accessScript[] = [
    { executable: "BruteSSH.exe", func: ns.brutessh },
    { executable: "FTPCrack.exe", func: ns.ftpcrack },
    { executable: "relaySMTP.exe", func: ns.relaysmtp },
...
export function getBestTarget(ns: NS): string {
  ...
  for (const target of targets) {
    if (ns.getServerRequiredHackingLevel(target) < ns.getHackingLevel() / 2) {
      const currWeight =
        ns.getServerMaxMoney(target) / ns.getServerMinSecurityLevel(target);
